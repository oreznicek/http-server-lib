...
class RequestParser {
    ...
    std::expected<RequestHeader, ServerErr> parse_header(std::string_view line) noexcept;
    std::expected<Request, ServerErr> parse_headers(std::string_view headers) noexcept;
public:
    RequestParser(std::size_t headers_limit, std::size_t body_limit, std::size_t request_target_limit);
    std::expected<Request, ServerErr> parse_request(Connection& conn) noexcept;
};
